...
// Trie for uppercase english letter A~Z
public class Trie implements Iterable<String> {
    private static final int R = 26;        // A~Z
    static final Alphabet alph = Alphabet.UPPERCASE;

    private Node root;      // root of trie
    private int n;          // number of keys in trie
    
    private Node last; // cache, last search history (hasPrefix function and contain)
    // set by hasPrefix 
     ...
     */
    public boolean contains(String key, boolean history) {
    ...
    private Node get(Node x, String key, int d) {
        ...
        //char c = key.charAt(d);
        int c = alph.toIndex(key.charAt(d));
    ...
    // check if the trie contain keys with prefix pre
    // history: assume pre is one more character than the string represented by history
    public boolean hasPrefix(String pre, boolean history) {
        if (history && last != null) { // turn on history
            char c = pre.charAt(pre.length()-1);
            int i = alph.toIndex(c);
            Node x = last.next[i];
            last = x; // update search history
            if (x != null) {
                return true;
            }
            else
            ...
            last = x; // update save search history
            if (x != null) {
                return true;
            }
            else
    ...
    private int longestPrefixOf(Node x, String query, int d, int length) {
        ...
        int c = alph.toIndex(query.charAt(d));
